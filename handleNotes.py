...
def getPublicNotes(param):
    if (request.method == "GET"):
        notes = db.session.execute(text(
            'Select * from (select * from notes where notes.notePublic=1 and notes.inArchived=1) as b inner join datas on b.idNote=datas.idNote'.format(param)))
